...
Specific Steps:
1. Use the BBox Label Tool to draw a box around a handicap parking pass in images of cars.
2. Convert the coordinates returned by the BBox Label Tool to the correct YOLO format. 
3. Separate these files into a training set and a test set.
4. Create two configuration files for YOLO: obj.data and obj.names. Adjust yolo-obj.cfg (the third configuration file) to match your
settings and number of classes.
5. Use these 3 configuration files and the training set to train YOLO to identify handicap parking passes. Do this by typing the following
command into the terminal: darknet.exe detector train cfg/obj.data cfg/yolo-obj.cfg darknet19_448.conv.23
6. Test the accuracy of your program with the test set.
7. Use the pre-trained version of YOLO to identify cars in video footage. If a car is present in the video footage for an extended amount
of time, assume it is a parked car. Only look for handicap parking passes in parked cars.
8. Code methods that record statistics such as the percentage of cars that park illegally in a specific handicap parking space each day.
